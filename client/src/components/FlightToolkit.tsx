...
function MissionReadinessTool({ profile, voltageNominal, batteryContinuousA }: Props) {
  ...
  const fixReason = (id: string) => {
    const target = document.getElementById(`readiness-input-${id}`);
    if (!target) return;
    target.scrollIntoView({ behavior: "smooth", block: "center" });
    if (target instanceof HTMLInputElement || target instanceof HTMLButtonElement) target.focus({ preventScroll: true });
    target.classList.add("is-fix-target");
    window.setTimeout(() => target.classList.remove("is-fix-target"), 1400);
  };
  return <div className="readiness-tool">
    ...
    <div className="readiness-grid">
      <div className="readiness-inputs"><div className="panel-label">LIVE SIGNAL INPUTS</div><NumberField id="readiness-input-sag" label="Pack resistance" value={resistance} unit="mΩ" onChange={setResistance} /><NumberField id="readiness-input-thrust" label="Static thrust / motor" value={thrust} unit="g" onChange={setThrust} /><div className="readiness-hint"><ShieldCheck size={15} /> Inputs stay local and missing thrust data never gets guessed.</div></div>
      <div className="readiness-breakdown" id="readiness-input-power"><div className="panel-label">READINESS BREAKDOWN</div><ReadinessBar label="Power headroom" value={result.factors.power} max={25} detail={`${result.power.continuousHeadroomA.toFixed(1)} A`} /><ReadinessBar label="Voltage sag" value={result.factors.sag} max={20} detail={`${result.sag.sagV.toFixed(2)} V`} /><ReadinessBar label="Thrust evidence" value={result.factors.thrust} max={25} detail={result.thrust.ratio === null ? "not supplied" : `${result.thrust.ratio.toFixed(2)} : 1`} /><ReadinessBar label="Field checklist" value={result.factors.checklist} max={30} detail={`${checksComplete}/${preflightItems.length}`} /></div>
    </div>
    <section className="readiness-reasons" aria-labelledby="readiness-reasons-title"><div className="readiness-reasons__head"><div><div className="panel-label">DECISION TRACE / WHY THIS STATUS</div><h3 id="readiness-reasons-title">What is holding the verdict here?</h3></div><span>{result.reasons.filter((reason) => reason.state !== "pass").length} gate{result.reasons.filter((reason) => reason.state !== "pass").length === 1 ? "" : "s"} active</span></div><div className="readiness-reasons__list">{result.reasons.map((reason) => <div className={`readiness-reason ${reason.state}`} key={reason.id}><i /><div><strong>{reason.label}</strong><small>{reason.status} · {reason.points}/{reason.maxPoints} pts</small><p>{reason.detail}</p><em>{reason.action}</em>{reason.state !== "pass" ? <button className="fix-this" onClick={() => fixReason(reason.id)}>Fix this <ArrowRight size={12} /></button> : null}</div></div>)}</div></section>
    <div className="readiness-footer"><span><Target size={15} /> Conservative by design</span><small>Green means the inputs are ready for a field check — not a certification of airworthiness.</small><button id="readiness-input-checklist" onClick={() => setChecked(Object.fromEntries(preflightItems.map((item) => [item.id, true])))}>Mark checklist complete <ArrowRight size={13} /></button></div>
  </div>;
}
...
function NumberField({ id, label, value, unit, onChange, readOnly = false }: { id?: string; label: string; value: number; unit: string; onChange: (value: number) => void; readOnly?: boolean }) { return <label className="tool-field"><span>{label}</span><div><input id={id} type="number" value={Number.isFinite(value) ? value : 0} readOnly={readOnly} onChange={(e) => onChange(num(e.target.value))} /><b>{unit}</b></div></label>; }
