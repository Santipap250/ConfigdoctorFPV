...
function ToolCenter({ groups, onOpen, profile, metrics }: { groups: typeof toolGroups; onOpen: (active: boolean, name: string) => void; profile: DroneProfile; metrics: ReturnType<typeof calculateMetrics> }) { return <div className="tools-page page-enter"><header className="mission-header"><div><div className="eyebrow"><i />SYSTEM / TOOL CENTER</div><h1>Tools with a <em>clear scope.</em></h1><p>The toolkit below is designed around the jobs FPV pilots repeatedly need: power budgeting, battery sag, thrust margin, field pre-flight checks, and config comparison.</p></div><div className="tool-search"><Search size={17} /><input aria-label="Search tools" placeholder="Search tools" /></div></header><FlightToolkit profile={profile} voltageNominal={metrics.voltageNominal} batteryContinuousA={metrics.batteryContinuousA} /><div className="tool-groups">{groups.map((section) => <section key={section.group} className="tool-group"><div className="tool-group__head"><span>{section.group}</span><i /></div><div className="tool-grid">{section.tools.map(({ name, detail, active, icon: Icon }) => <button className={`tool-card ${active ? "tool-card--active" : ""}`} onClick={() => onOpen(active, name)} key={name}><span className="tool-card__icon"><Icon size={20} /></span><div><h2>{name}</h2><p>{detail}</p></div>{active ? <StatusBadge level="pass">AVAILABLE</StatusBadge> : <StatusBadge level="neutral">PLANNED</StatusBadge>}<ChevronRight className="tool-card__arrow" size={18} /></button>)}</div></section>)}</div></div>; }
